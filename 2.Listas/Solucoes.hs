...
qtdElementosLista1 = 2

-- Quantos elementos existem na lista [[2,3]]?
qtdElementosLista2 :: Int
qtdElementosLista2 = 1
...
{-
Sua resposta aqui: Lista de Lista de Inteiro
-}
...
sumPairs [] = []
sumPairs ((a,b):tl) = (a+b):sumPairs tl
...
books [] _ = []
books ((p,b):tl) borrower
    | borrower == p = b:books tl borrower
    | otherwise = books tl borrower

-- Retorna as pessoas que pegaram um determinado livro emprestado
borrowers :: Database -> Book -> [Person]
borrowers [] _ = []
borrowers ((p,b):tl) borrowedBook
    | b == borrowedBook = p:borrowers tl borrowedBook
    | otherwise = borrowers tl borrowedBook

-- Verifica se um livro está emprestado
borrowed :: Database -> Book -> Bool
borrowed [] _  = False
borrowed ((p,b):tl) borrowedBook
    | b == borrowedBook = True
    | otherwise = borrowed tl borrowedBook
...
returnLoan [] _ _ = []
returnLoan ((borrower, borrowedBook):tl) p b 
    | (borrower, borrowedBook) == (p,b) = tl
    | otherwise = (borrower, borrowedBook):returnLoan tl p b
...
memberComp l e = length [f|f<-l,f==e] > 0

booksComp :: Database -> Person -> [Book]
booksComp db p = [borrowedBook|(borrower,borrowedBook)<-db,borrower==p]

borrowersComp :: Database -> Book -> [Person]
borrowersComp db b = [borrower|(borrower,borrowedBook)<-db,borrowedBook==b]

borrowedComp :: Database -> Book -> Bool
borrowedComp db b = length (borrowersComp db b) > 0 

returnLoanComp :: Database -> Person -> Book -> Database
returnLoanComp db p b = [(borrower,borrowedBook)|(borrower,borrowedBook)<-db,(borrower,borrowedBook)/=(p,b)]
