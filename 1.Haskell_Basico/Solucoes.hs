...
-- 5) Função que dado um valor inteiro s (sales) e um número de semanas n, 
-- retorna quantas semanas de 0 a n tiveram venda igual a s.
quantasSemanasVenda :: Int -> Int -> Int
quantasSemanasVenda s 0
  | sales 0 == s = 1
  | otherwise = 0
quantasSemanasVenda s n
  | sales n == s = 1 + quantasSemanasVenda s (n - 1)
  | otherwise = quantasSemanasVenda s (n - 1)
...
averageSales n = (fromIntegral (sales n) + fromIntegral n * averageSales (n - 1))/fromIntegral (n + 1)
