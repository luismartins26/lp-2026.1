...
ordenaDecrescente [] = []
ordenaDecrescente (h:tl) = [e|e<-ordenaDecrescente tl,e>=h] ++ [h] ++ [e|e<-ordenaDecrescente tl,e<h]

-- b) Retorna um valor booleano indicando se a lista é decrescente ou não.
isDecrescente :: [Int] -> Bool
isDecrescente l = l == ordenaDecrescente l
...
percorreInfix NilT = []
percorreInfix (Node n l r) = percorreInfix l ++ [n] ++ percorreInfix r
